...
- [ ] **T10.1** Write `scripts/audit_local_binding.py` detecting Type 1 candidates (`patch("src.X.Y")` where X imports Y locally into a different module).
  Requirements:
...
- [ ] **T10.2** Add `rebind_local(module, attr, target)` helper in `tests/conftest.py` with docstring linking `adb531c / 6b41335`.
  Requirements:
...
- [ ] **T10.4** Write `CONTRIBUTING.md` "Mock contract rules" section naming the three iceberg types with canonical commit links.
  Requirements:
...
- [ ] **T10.5** Write `docs/test-mock-iceberg-taxonomy.md` with one example per type (pre-fix / post-fix diff excerpt).
  Requirements:
