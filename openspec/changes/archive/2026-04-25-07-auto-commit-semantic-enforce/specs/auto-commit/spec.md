# Spec: Auto-Engineer Commit Semantic Enforcement

## Summary

Every commit authored by the auto-engineer runtime MUST conform to
Conventional Commit (T-COMMIT-SEMANTIC-GUARD, v3), and that conformance
MUST be enforced **inside the auto-engineer runtime** rather than via
`.git/hooks/commit-msg` (currently blocked by the `index.lock` posture
recorded in `T-ACL-STATE-RECALIBRATE`). This change deletes the silent-drift
loophole that produced 7 violations in 48 hours.
