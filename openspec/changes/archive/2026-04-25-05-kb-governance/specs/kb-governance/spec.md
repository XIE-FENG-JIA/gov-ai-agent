# Spec: Knowledge-Base Governance Boundary

## Summary

This change defines the repo-owned governance contract for deciding which
corpus entries are eligible for active retrieval, how live ingest handles
fixture fallback, and how operators verify an `--only-real` rebuild before the
rewritten document flow trusts the index again.
