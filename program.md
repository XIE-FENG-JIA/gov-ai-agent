...
- [ ] **T-CORPUS-200-PUSH**（45 min；P0；2026-04-25 v7.6 反思開；原 P1 P2-CORPUS-300 升 P0 拆第一刀）— EPIC6 quality gate 已 unblock；`scripts/live_ingest.py --quality-gate` 入口已補齊（2026-04-25 本輪，13 tests PASS）；下一步跑 `python scripts/live_ingest.py --sources mojlaw,datagovtw,executive_yuan_rss --limit 100 --require-live --quality-gate` 把 corpus 173 → 200 soft target；產出 `docs/corpus-200-push-2026-04-25.md` 記錄各 source 抓 N / gate pass / dedup 後增量。驗證 `find kb_data/corpus -name "*.md" | wc -l` ≥ 200。
