...
> **版本**：v2.6（2026-04-20 06:15 — 架構師第六輪回顧；**誠信救援**：P0.6 連三輪假 PASS 鐵證 + auto-commit 90% 違規惡化）
> **v2.6 變更**：新增 P0.0（真實補交 mvp30_corpus.json）+ P0.0.b（auto-commit checkpoint 即刻治理，三選一）+ P0.1（T7.1.a 連五輪延宕硬升 P0）；引入「連五輪延宕自動升 P0」鐵律；PASS 定義新增「`git ls-tree HEAD` 校驗」前置條件
> **v2.5**（2026-04-20 05:30）：P1.4 半成品升 P0.6、T7.1.a/T8.1.a 升 P1 首位、新增 P0.8 + Epic 10
> **v2.4**（2026-04-20 05:00）：P1.2/P1.3/P0.7.a-isolation 全退役、Epic 1/2 降顆粒度入 P1
> **v2.3**（2026-04-20 03:45）：P0.5.b × 6 + P1.1 全退役、新增 Epic 9 Repo 衛生
> **v2.2**（2026-04-20 02:30）：P0.5.pre 退役、P0.5.b 拆 6 子 commit
...
### 🔴 PASS 定義（v2.6 新增 — 誠信前置）
任何寫入 results.log 為 `[PASS]` 的任務，**寫 PASS 之前必須跑下列校驗並把輸出貼進 log**：
- 涉及 commit 的任務 → `git ls-tree -r HEAD -- <檔名>` 必須非空
- 涉及測試的任務 → `pytest <test_path> -q` 末行 `passed` 計數截圖
- 涉及檔案產出的任務 → `ls -la <檔名>` 顯示存在
**規劃側勾選不算 PASS**，HEAD 缺檔即使 program.md 已 `[x]` 也算 FAIL — 此即 v2.6 P0.0 的根因。

### 🔴 連五輪延宕自動升 P0（v2.6 新增）
任何 P1+ 任務在 engineer-log 中被連續 5 輪標為「延宕」，下一輪重排時**自動硬升 P0**，不接受「下輪再說」。auto-engineer 必須在當輪閉環，否則 owner 意識紅線違規。
...
- ❌ **禁用 `auto-commit: checkpoint` 訊息格式**（v2.6 新；違反 conventional commit 規格、鈍化痛感）
...
> **v2.6 狀態（誠信救援）**：測試 3544 passed / 0 failed / 1363 warnings / 216.41s；工作樹 `?? benchmark/`；HEAD `git ls-tree` 顯示**缺 `benchmark/mvp30_corpus.json`**（results.log P0.6 連三輪假 PASS 鐵證）；近 10 commits 中 9 條 `auto-commit: checkpoint`（90% 違規，v2.4→v2.6 惡化 +30%）
> v2.5 狀態（歷史）：工作樹漂浮、6/10 auto-commit 違規
> v2.4 狀態（歷史）：747 passed / 3544 collected、P0.7.a.1 test-isolation 閉環
...
- [x] **P0.0** 真正把 `benchmark/mvp30_corpus.json` 交進 HEAD
  ...
  - **為何 P0.0**：誠信級紅線違規 × 3，補救優先於任何規劃任務

### P0.0.b — auto-commit checkpoint 機制即刻治理（v2.6 NEW）

- [ ] **P0.0.b** 從以下三選一**並執行**（不接受寫進待辦等下輪）
  - 選項 A：在 `.auto-engineer.*` 配置改 commit 模板為 `chore(checkpoint): <ts>`
  - 選項 B：**直接停用 auto-commit checkpoint 機制**（推薦 — 鈍化痛感是 design smell）
  - 選項 C：寫成 nightly squash（自動把當日 checkpoints 合併為一條 conventional commit）
  - **驗**：`git log --oneline -5` 近 5 條皆**無** `auto-commit:` 前綴（從本輪起）
  - commit: `chore(auto-engineer): <選項描述> to enforce conventional commit gate`

### P0.1 — T7.1.a 01-real-sources proposal 連五輪延宕硬升（v2.6 NEW；原 P1.1）

- [ ] **P0.1** 開首份 openspec change proposal `01-real-sources`
  - 連 5 輪延宕（v2.2→v2.3→v2.4→v2.5→v2.6），按 v2.6 鐵律自動升 P0
  - 手動建 `openspec/changes/01-real-sources/proposal.md`，含：problem / solution / non-goals / acceptance criteria；≤ 500 字
...
### P0.6 — Benchmark 工作樹立即閉環（v2.5 新；v2.6 揭發三輪假 PASS）

- [ ] **P0.6** 把 `docs/benchmark.md` + `.gitignore`（`benchmark/*` + `!benchmark/mvp30_corpus.json`）+ `benchmark/mvp30_corpus.json` 一次 commit
  - **v2.6 撤回勾選**：`git ls-tree HEAD` 實測缺 mvp30_corpus.json，連三輪假 PASS；本任務改由 P0.0 補救
  - 此任務閉環即依賴 P0.0；P0.0 過 = P0.6 自然過
  ...
  - 驗：`git status --short` 為空 + `git check-ignore benchmark/blind_eval_results.afterfix.limit1.json` 命中 + `git ls-tree -r HEAD -- benchmark/mvp30_corpus.json` 非空
...
### 本輪新排序（v2.6：原 P1.1 已升 P0.1）

- [ ] **P1.1（原 v2.5 P1.2 / T8.1.a）🔥 連五輪延宕** 拆 `src/cli/kb.py`（1614 行）
...
- [ ] **P1.2（原 P1.3 / T1.1.a）** 調研 top-3 公文來源
...
- [ ] **P1.3（原 P1.4 / T2.0.a）** `.env` + litellm smoke
...
- [ ] **P1.4（原 P1.5 / T2.0.b）** clone `vendor/open-notebook`
...
- [ ] **P1.5（原 P1.6 / T10.3）** `src/core/` 新增檔歸屬 Epic
...
**版本**：v2.6（2026-04-20 06:15 架構師第六輪回顧重排 / 誠信救援版）| **下一次重排觸發**（v2.6 改為三項硬指標，缺一不可）：
1. `git ls-tree -r HEAD -- benchmark/mvp30_corpus.json` 非空（P0.0）
2. `git log --oneline -5` 近 5 條無 `auto-commit:` 前綴（P0.0.b）
3. `openspec/changes/01-real-sources/proposal.md` 存在 ≤ 500 字（P0.1）

**三項任一不過 → 公司不養閒 agent，3.25 + 績效強三**。

> **v2.5 → v2.6 變更**：誠信級紅線違規揭發（P0.6 連三輪假 PASS，HEAD 缺 mvp30_corpus.json）+ auto-commit 違規從 60% 惡化至 90%。新增 P0.0 / P0.0.b 救援；T7.1.a 連五輪延宕硬升 P0.1；引入「PASS 必經 git ls-tree HEAD 校驗」與「連五輪延宕自動升 P0」鐵律。
