...
- [x] **T-FAT-ROTATE-V2（刀 4）** ✅（2026-04-21 22:56）— `src/api/routes/agents.py 503` 已抽出 `src/api/routes/_agents_parallel.py 117`，主檔降到 **397**；保留 `src.api.routes.agents._run_format_audit` patch 面與所有 API route 契約，並把 parallel review 聚合邏輯移到 helper module；驗證 `python -m pytest tests/test_stress.py -q -k "concurrent_parallel_review_requests"` = **1 passed**、`python -m pytest tests/test_api_server.py -q` = **259 passed**、`python -m pytest tests/ -q --ignore=tests/integration -x` = **3741 passed / 0 failed**
...
- [x] **T-FAT-ROTATE-V2（刀 5）** ✅（2026-04-21 23:35）— `src/api/middleware.py 469` 已拆為 `src/api/middleware.py 237 + src/api/_middleware_rate_limit.py 77 + src/api/_middleware_metrics.py 54 + src/api/_middleware_body_limit.py 79`；保留 `RequestBodyLimitMiddleware` / `_RateLimiter` / `_MetricsCollector` / `rate_limiter` / `metrics` / `security_middleware` 對外 patch 面；驗證 `python -m pytest tests/ -q --ignore=tests/integration -x` = **3741 passed / 0 failed**
- [ ] **T-FAT-ROTATE-V2（刀 6+）** 🟡 **v6.2 P1（輪拆；連 2 輪延宕 = 3.25）** — 剩 **5** 胖檔 > 400：`api-models 461 / generate/export 459 / fact_checker 446 / datagovtw 410 / workflow_cmd 406`；SOP 第 16/17/18/19/20 刀輪到哪刀哪；單輪只切 1 檔避免顆粒度漂移
...
- [x] **T5.4 / only-real post-rebuild verification** ✅（2026-04-21 22:00）— `src/cli/kb/rebuild.py` 新增 `--verify-docx`，only-real 重建後可直接驗證 DOCX citation metadata 是否仍只解析到 active corpus；`src/cli/verify_cmd.py` 抽出可重用 verification helper，並補 `tests/test_cli_commands.py`、`tests/test_corpus_provenance_guard.py` 回歸，鎖住 fixture-backed repo evidence 不可混入驗證；驗證 `python -m pytest tests/test_cli_commands.py tests/test_corpus_provenance_guard.py tests/test_e2e_rewrite.py -q -k "verify or rebuild or provenance"` = **16 passed**
...
- [x] **T-FAT-ROTATE-V2（刀 1）** ✅ 新胖八檔首刀已閉：`config_tools 585` → `config_tools.py 257 / config_tools_mutations_impl.py 225 / config_tools_fetch_impl.py 96`；patch 面與 CLI 入口維持不變
  ...
  - **產出**：維持 `src.cli.config_tools` 單一 import 錨點，把重邏輯搬到 `src/cli/config_tools_mutations_impl.py` 與 `src/cli/config_tools_fetch_impl.py`
  - **驗 1**：`wc -l src/cli/config_tools*.py` = **257 / 225 / 96**（每檔 ≤ 400）
  - **驗 2**：`python -m pytest tests/test_config_tools_extra.py -q --no-header` = **29 passed**
  - **驗 3**：`python -m pytest tests/test_cli_commands.py -q --no-header -k "config_tools or fetch_models or test_connectivity or restore or backup"` = **33 passed**
...
- [x] **T-FAT-ROTATE-V2（刀 4）** ✅（2026-04-21 22:56）`src/api/routes/agents.py 503` → `src/api/routes/agents.py 397 + src/api/routes/_agents_parallel.py 117`；保留 `src.api.routes.agents._run_format_audit` patch 面，`parallel_review` route 契約不變；`tests/test_stress.py -k "concurrent_parallel_review_requests"`、`tests/test_api_server.py -q`、全量 `tests/ -q --ignore=tests/integration -x` 全綠
- [x] **T-FAT-ROTATE-V2（刀 5）** ✅（2026-04-21 23:35）`src/api/middleware.py 469` → `src/api/middleware.py 237 + src/api/_middleware_rate_limit.py 77 + src/api/_middleware_metrics.py 54 + src/api/_middleware_body_limit.py 79`；`RequestBodyLimitMiddleware` / `_RateLimiter` / `_MetricsCollector` / `rate_limiter` / `metrics` / `security_middleware` patch 面不變；全量 `tests/ -q --ignore=tests/integration -x` = **3741 passed / 0 failed**
- [ ] **T-FAT-ROTATE-V2（刀 6+）** 🔴 下輪再鎖 `api-models 461 / generate/export 459 / fact_checker 446 / datagovtw 410 / workflow_cmd 406`
...
- [x] **T5.1（2026-04-21）** centralize active corpus eligibility rule
  - **完成（2026-04-21 18:23）**：新增 `src/knowledge/corpus_provenance.py`，把 `synthetic` / `fixture_fallback` / `deprecated` 的 active corpus eligibility 規則收斂成單一 repo-owned helper，並讓 `src/cli/kb/rebuild.py`、`src/e2e_rewrite.py`、`src/cli/verify_cmd.py` 共用；驗證 `python -m pytest tests/test_corpus_provenance_guard.py tests/test_cli_commands.py tests/test_e2e_rewrite.py -q -k "verify or kb_rebuild or provenance or load_real_corpus"` = 12 passed，`python -m pytest tests/ -q --ignore=tests/integration -x` = 3736 passed / 0 failed。
- [x] **EPIC5-T5.3（2026-04-21）** `kb rebuild --only-real` operational path
  - **完成（2026-04-21 19:10）**：`src/cli/kb/rebuild.py` 的 `--only-real` 由 legacy `examples/regulations/policies` 改為優先重建 `kb_data/corpus` active corpus；`doc_type=法規` 進 `regulations`，其餘真實公開來源進 `policies`，並新增 provenance rollup（`匯入 real` / `跳過 synthetic` / `fixture_fallback` / `deprecated`）。若找不到 `kb_data/corpus` 才 loud fallback 回 legacy collections。驗證 `python -m pytest tests/test_cli_commands.py tests/test_knowledge.py -q -k "kb_rebuild or stats"` = 19 passed，`python -m pytest tests/ -q --ignore=tests/integration -x` = 3738 passed / 0 failed。
