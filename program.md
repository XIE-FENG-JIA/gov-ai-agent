...
- [ ] **T-PYTEST-RUNTIME-FIX-v3**（P1；2026-04-25）— 本輪 **cross-session cold-start = 152.98s / 3801 passed**（新 session 獨立起跑，**破 ≤ 200s 目標 47s 裕量，BM25 cap 真效確認**）；**升級目標 ≤ 120s**（差 33s，待 `test_safe_score / meeting_exporter` 同類患者掃完）；守穩條件：下輪起手 cold-start > 180s 即 regression 回查。2026-04-25 本輪補丁：(1) `tests/conftest.py` commit `23802ec` 遺留 orphaned mock_llm body 修復（234 errors → 0）；(2) `src/knowledge/_manager_hybrid.py` `_bm25_search` jieba cold-start early-return（empty KB 跳過 5-7s jieba init）；(3) pytest-xdist 3.8 安裝 + `pyproject.toml` `addopts = "-v -n auto"` 並行化 → **146.67s / 3889 passed（-n auto, 20 workers）**；≤ 120s upgrade target 差 26.67s（3889 tests +88 vs 3801 baseline）。
...
- [x] **T-SYNTHETIC-AUDIT**（P1；2026-04-25 新；v7.3 sensor 首入；script 閉）— `scripts/audit_synthetic_flag.py`（97 行）建立；`tests/test_audit_synthetic_flag.py` = 18 passed；`python scripts/audit_synthetic_flag.py` 顯示 155/192 tagged，37 份未標 `synthetic:` 待補；`--strict` exit 0 需補完 37 份後才通過；與 T-LIQG-1 `SyntheticContamination` 檢測契合，為 CORPUS-300 擴量掃雷。
