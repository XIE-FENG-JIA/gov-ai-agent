...
> **版本**：v2.7（2026-04-20 07:00 — 架構師第七輪；**根因回歸**：`.git` ACL DENY 鎖定為真兇，v2.6 三項硬指標 0/3 全 FAIL）
> **v2.7 變更**：
> - **根因升級**：`.git` 外來 SID DENY(W,D,Rc,DC) 是所有 commit 任務被阻斷的真因（results.log 17 實測）→ P0.0 從「補交檔案」改寫為「解除 `.git` ACL DENY」，標 🛑 需人工 Admin
> - **新 ACL-gated 規則**：ACL 解除前，commit 類任務全體凍結；auto-engineer 只執行 **read-only 任務**（規劃、調研、文件）避免再產假 PASS
> - **Read-only 任務前置**：P0.2-P0.4 全為 read-only（文件、調研、盤點），繞開 commit 瓶頸
> - **大量去重**：P0.6/P0.8/Epic 7 T7.1.a/Epic 8 T8.1/Epic 10 T10.1+T10.3/Epic 1 T1.1.a 已分別與 P0 或 P1 合併，原位留指向註記
> - **Epic 10 新增 T10.4**：auto-engineer 偵測 ACL DENY → 切 read-only 模式
> - v2.6 三項硬指標實測：(a) `git ls-tree HEAD` 仍空 (b) 近 5 條仍全是 `auto-commit:` (c) proposal 未建 → 0/3
...
> **v2.7 狀態（根因回歸）**：測試 3544 passed / 0 failed；工作樹 `?? benchmark/`；`.git` 存在 SID `S-1-5-21-541253457-...-692795393` DENY(W,D,Rc,DC)；v2.6 三硬指標 **0/3 全 FAIL**；近 5 commits 100% `auto-commit:`（v2.6 惡化 10%）
> v2.6 狀態（歷史）：測試 3544、HEAD 缺 mvp30_corpus.json、auto-commit 90%
> v2.5 狀態（歷史）：747 passed / 3544 collected、P0.7.a.1 閉環

### P0.0 — 🛑 解除 `.git` 外來 SID DENY ACL（v2.7 REDEFINED — 真根因）

- [ ] **P0.0** 🛑 需人工 Admin：移除 `.git` 對 SID `S-1-5-21-541253457-2268935619-321007557-692795393` 的 DENY ACL
  ...
  - **BLOCKER 範圍**：此題未過 → P0.1 / P0.5 / P1.1 / P1.3 / P1.4 全體 BLOCKED
  - commit（解除後）：`chore(repo): remove foreign SID DENY ACL on .git`

### P0.1 — auto-commit checkpoint 機制治理（v2.7；原 P0.0.b + P0.8 合併）

- [ ] **P0.1** 🚦 ACL-gated：選項三選一並執行
  - 現況：近 5 commits 100% `auto-commit: auto-engineer checkpoint (...)`，違反 conventional commit 鐵律 + 鈍化痛感
...
### P0.2 — ✅ read-only：Disaster Recovery 文件（v2.7 NEW — ACL 根因存檔）

- [x] **P0.2** ✅ 不依賴 ACL：寫 `docs/disaster-recovery.md`
  - 內容：(1) `.git` ACL DENY 事故始末（v2.4→v2.7 連四輪）；(2) SID 揭露 + icacls 解除 SOP；(3) `meta_git/` / `meta_git_live/` / `recovered_repo/` / `git_safe/` / `repo_meta/` / `meta_test/` 去留決策流程；(4) `.git_acl_backup.txt.quarantine-*` 處置；(5) 未來 agent 遇 ACL DENY 的 read-only fallback 流程
  - **驗**：`ls docs/disaster-recovery.md && wc -w docs/disaster-recovery.md` ≥ 400 字
  - 吸收：原 P0.7.a.3 / P0.7.b / T9.4.b 的 doc 部分
  - commit（ACL 解除後）: `docs(ops): add disaster recovery playbook for .git ACL deny + backup dirs`

### P0.3 — ✅ read-only：top-3 公文來源調研（v2.7；原 P1.2 升首）

- [ ] **P0.3** ✅ 不依賴 ACL：寫 `docs/sources-research.md` 首版
...
### P0.4 — ✅ read-only：src/core 失控檔 Epic 歸屬盤點（v2.7；原 P1.5 升首）

- [ ] **P0.4** ✅ 不依賴 ACL：`src/core/` 新增 5 檔寫入 program.md 尾段
  ...
  - **驗**：`grep -c "失控檔盤點" program.md` >= 1 + 後接 5 條
  - commit（ACL 解除後）: `docs(program): inventory uncategorized src/core/ additions`

### P0.5 — 🚦 ACL-gated：Open 01-real-sources change proposal（v2.7；原 P0.1）

- [ ] **P0.5** 🚦 依賴 P0.0：寫 `openspec/changes/01-real-sources/proposal.md`
  - 連 5 輪延宕（v2.2→v2.6）；v2.7 確認延宕原因為 ACL（非意願）→ 保 P0 但 ACL-gated
  - 內容：problem / solution / non-goals / acceptance criteria，≤ 500 字
  - **驗**：`ls openspec/changes/01-real-sources/proposal.md && wc -w` ≤ 500
...
- [x] **P0.6（v2.5）** 已回滾勾選 → 併入 v2.7 P0.0（HEAD 仍缺檔，ACL 阻擋為真因）
- [x] **P0.7.a.1** CLI cwd per-test 隔離（v2.4 閉環）
- [x] **P0.7.a.2** root tmp orphan 自然清零（v2.5 退役）
- ~~P0.0（v2.6 補交檔案）~~ → v2.7 改寫為解 ACL（補檔是症狀）
- ~~P0.0.b（v2.6 auto-commit 治理）~~ → v2.7 併入 P0.1
- ~~P0.8（v2.5 conventional 前綴）~~ → v2.7 併入 P0.1
- ~~P0.7.a.3 / P0.7.b（quarantine / backup dirs 去留）~~ → v2.7 併入 P0.2 disaster-recovery
- ~~P0.6（v2.5 benchmark 閉環）~~ → v2.7 併入 P0.0（ACL 過後自然閉）
...
**版本**：v2.7（2026-04-20 07:00 架構師第七輪 / 根因回歸版）

**下一輪重排觸發**（v2.7 五項硬指標，依執行順序）：
1. `icacls .git 2>&1 | grep -c DENY` == 0（P0.0；根因）
2. `git log --oneline -5 | grep -c "auto-commit"` == 0（P0.1）
3. `ls docs/disaster-recovery.md` 存在 ≥ 400 字（P0.2；read-only 可先做）
4. `ls docs/sources-research.md` 存在 ≥ 3 個 `## ` 節（P0.3；read-only 可先做）
5. `grep -c "失控檔盤點" program.md` ≥ 1 且後接 5 條（P0.4；read-only 可先做）

**read-only 三項（P0.2-P0.4）任一不過 = 公司不養閒 agent，3.25 + 績效強三**（ACL 不是藉口，read-only 任務本輪就能落）。

> **v2.6 → v2.7 變更**：根因從「造假意願」回歸至「`.git` 外來 SID DENY ACL 系統層阻斷」；新增 ACL-gated 任務規則 + read-only 任務池；P0.2-P0.4 升首位不依賴 ACL；P0.6 / P0.8 / Epic 7 T7.1.a / Epic 8 T8.1 / Epic 10 T10.1+T10.3 / Epic 1 T1.1.a 去重合併。auto-engineer 本輪起在 read-only 模式下仍可進 3 條硬指標；ACL 由人工 Admin 解除後自然解鎖 commit 類任務。
