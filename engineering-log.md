...
**做了什麼**:
- `validators.py`: 單字語氣詞改用 regex 匹配句尾位置（後接標點/空白/文末），多字口語詞維持原邏輯
- `validators.py`: 術語檢查改為長名優先排序，短名若為已匹配長名的子字串則跳過
- 修正 `test_validators_coverage.py` 的 `test_no_ref_section` 測試文本（意外含「參考來源」子字串的預存 bug）
- 新增 11 個測試：9 個假陽性驗證 + 2 個重複報告驗證
**結果**: PASS — 224 passed, 零回歸
**下一步可能**:
- 編輯器基於結構化 suggestion 實作「一鍵套用」自動修正
- 批次處理效能優化（MISSION 功能缺口）
...
### [2026-03-27] Round 10 — 新功能：gov-ai cite 法規引用建議命令
**角度**: ✨ 功能缺口（MISSION「自動引用正確的法規依據」直接對應）
**為什麼**: 連續 2 輪（Round 9）修測試/bug，強制切換到新功能。第一性原理分析：MISSION 第 2 條「自動引用正確的法規依據」始終沒有專屬 CLI 入口——`regulation_doc_type_mapping.yaml`（144 行、20+ 部法規）與 64 份真實法規全文早已在 kb_data，但使用者沒有任何工具能直接查詢「這份公文應引用哪些法條」，必須自行翻閱映射表，完全是體驗缺口。
**搜尋**: WebSearch 「台灣政府公文法令依據引用格式 行政院公文處理手冊 法條引用規範」——確認公文法令依據欄位標準格式為「依據《XX法》第X條規定」；國發會《政府文書格式參考規範》105年版為現行依據；法規引用應標示法規名稱+條數，不需全文引錄。
**做了什麼**:
- `src/cli/cite_cmd.py`（新增）：11 類公文類型偵測規則（優先序排列防誤判）、載入 `regulation_doc_type_mapping.yaml`、篩選適用法規、Rich 彩色表格 + JSON 雙輸出模式、`--kb` 語意搜尋模式（KB 不可用時靜默降級）、`--format json` 供程式解析
- `src/cli/main.py`：註冊 `app.command("cite")`
- `tests/test_cite_cmd.py`（新增）：22 個測試——類型偵測 8 個、映射載入 3 個、適用篩選 6 個、CLI 整合 5 個
- 修正偵測規則 bug：`開會通知單` 原含「出席」關鍵字誤匹配「出席人員」→移除；`公告` 原含「主旨：」誤匹配函文→移除
**結果**: PASS — 22 新測試全通過；982 核心測試零回歸
**下一步可能**:
- 執行 `gov-ai kb sync` 讓 Round 73/77 新增的 12 筆範本正式生效
