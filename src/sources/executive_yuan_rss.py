"""Adapter for Executive Yuan RSS news feeds."""
...
class ExecutiveYuanRssAdapter(BaseSourceAdapter):
    """Adapter for the Executive Yuan public RSS feeds."""

    SOURCE_AGENCY = "行政院"
    FEED_URL = "https://www.ey.gov.tw/RSS_Content.aspx?ModuleType=3"
    DEFAULT_FIXTURE_PATH = Path(__file__).resolve().parents[2] / "tests" / "fixtures" / "executive_yuan_rss" / "feed.xml"
    ...
    def fetch(self, doc_id: str) -> dict[str, Any]:
        ...
        if normalized_id not in self._entry_cache:
            raise KeyError(f"Executive Yuan RSS entry not found: {normalized_id}")
