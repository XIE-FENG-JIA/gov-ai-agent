...
# 公文類型關鍵字自動偵測規則（優先序由高到低）
# 注意：每條規則只要符合清單中「任一」關鍵字就觸發，故越具體的規則應排越前面
_DETECT_RULES: list[tuple[list[str], str]] = [
    (["主持人", "出席人員", "決議事項"],           "會議紀錄"),   # 比 開會通知單 更具體，先判
    (["開會通知", "列席", "敬邀"],                 "開會通知單"),  # 移除泛用的「出席」
    (["訴願", "訴願人", "處分機關", "決定書"],     "訴願決定書"),
    (["採購公告", "廠商", "開標", "底價"],         "採購公告"),
    (["環評", "環境影響", "廢水", "空氣污染"],     "環保公告"),
    (["任命", "派任", "遷調", "免職", "任用"],     "人事令"),
    (["主文：", "茲公告", "茲依"],                 "公告"),        # 移除「主旨：」（函文也有）
    (["令", "茲令", "奉令", "依令"],               "令"),
    (["書函", "函請", "函覆", "查復"],             "書函"),
    (["擬辦：", "簽陳"],                           "簽"),          # 簽呈特有格式
    # 函是最常見的類型，最後兜底
...
def _detect_doc_type(text: str) -> str | None:
    """從草稿文字中自動偵測公文類型。"""
    for keywords, doc_type in _DETECT_RULES:
